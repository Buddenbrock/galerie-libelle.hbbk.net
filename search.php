...
?>
	...
	<?php
		...
		$abfrage = "SELECT * FROM artikel WHERE titel LIKE '%".$search_tag."%' or beschreibung LIKE '%".$search_tag."%' or artikelnummer LIKE '%".$search_tag."%'" ;
	...
	<section class="main exponate">
		<form action="intobasket.php" method="POST">
			...
			<p role="preis" class="exponate">
				<span><?=($preis) ?> &euro;</span>
				Einzelpreis: <?=($preis) ?> &euro;
			</p>
		</form>
	</section>
		...
		<section class="main exponate">
			<br /><br />
			<center><h2>Leider wurden keine Ergebnisse gefunden.<h2></center>
		</section>
<?php
		}
